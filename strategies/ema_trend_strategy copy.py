...
class EMATrendStrategy:
    ...
    def atr_stop_and_take_profit(self, df_15m, entry_price, direction):
        """
        计算ATR止损价和止盈价，严格5:1盈亏比，并打印调试信息
        direction: 'long' or 'short'
        """
        atr = self.calc_atr(df_15m, self.atr_period).iloc[-1]
        stop_distance = self.atr_mult * atr
        take_profit_distance = stop_distance * self.risk_reward_ratio  # 严格5:1
        if direction == 'long':
        ...
        print(f"[调试] 参数: atr_period={self.atr_period}, atr_mult={self.atr_mult}, risk_reward_ratio={self.risk_reward_ratio}")
    ...
    def should_open_long(self, df_1h, df_15m):
        ...
        满足1小时多头趋势+15分钟金叉
        """
        return self.htf_trend_filter(df_1h) == '多头趋势' and self.ltf_signal_realtime(df_15m) == '金叉'

    def should_open_short(self, df_1h, df_15m):
        """
        满足1小时空头趋势+15分钟死叉
        """
        return self.htf_trend_filter(df_1h) == '空头趋势' and self.ltf_signal_realtime(df_15m) == '死叉'
    ...
    def open_position(self, exchange, symbol, direction, leverage, risk_pct, entry_price=None, df_15m=None):
        ...
        try:
            ...
            # 用实际成交价计算止损止盈
            stop_price, take_profit_price, atr = self.atr_stop_and_take_profit(df_15m, executed_price, direction)
